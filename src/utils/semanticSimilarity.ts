/**
 * Semantic similarity between two tokens — no ML, no embeddings, just a lookup table
 * with a Jaccard character similarity fallback for unknown words.
 *
 * Why not use embeddings? This runs entirely in the browser with no extra dependencies.
 * The lookup table covers the most common synonym pairs that appear in model output diffs,
 * and Jaccard catches partial matches (e.g. "running" vs "runner") reasonably well.
 *
 * Scores are in [0, 1]:
 *   1.0 = identical
 *   ≥ 0.8 = treated as a paraphrase (see PARAPHRASE_THRESHOLD)
 *   < 0.8 = treated as a hard delete + insert in the diff
 */

// Hand-curated similarity scores for common synonym pairs.
// I kept antonyms in here too (e.g. fast/slow = 0.02) so the algorithm
// doesn't accidentally call opposites "similar".
const SIMILARITY_MAP: Record<string, Record<string, number>> = {
  ...
  great: { good: 0.88, excellent: 0.9, wonderful: 0.83 },
  ...
  desire: { want: 0.87, need: 0.8, wish: 0.78 },
  need: { want: 0.85, require: 0.89 },
  require: { need: 0.89, demand: 0.86 },
  help: { assist: 0.92, support: 0.88, aid: 0.9 },
...
// Tokens with similarity >= this are shown as "paraphrased" (yellow) in the diff
export const PARAPHRASE_THRESHOLD = 0.8

/**
 * Returns a similarity score in [0, 1] between two tokens.
 * Lookup table first, Jaccard fallback for unknown words.
 */
export function semanticSimilarity(a: string, b: string): number {
  ...
  // Check both directions since the map isn't always symmetric
  if (SIMILARITY_MAP[la]?.[lb] !== undefined) return SIMILARITY_MAP[la][lb]
  if (SIMILARITY_MAP[lb]?.[la] !== undefined) return SIMILARITY_MAP[lb][la]

  // Fall back to character-level Jaccard for words not in the table
  return jaccardSimilarity(la, lb)
}

/**
 * Jaccard similarity on character sets: |A ∩ B| / |A ∪ B|
 * Works surprisingly well for catching morphological variants
 * like "running" / "runner" or "helpful" / "helpfully".
 */
