...
function TokenSpan({ text, state, title }: { text: string; state: DiffToken['state']; title?: string }) {
  const styles: Record<DiffToken['state'], React.CSSProperties> = {
    equal: { color: 'var(--text-h)' },
    added: { background: 'rgba(34,197,94,0.15)', color: '#4ade80', borderRadius: '3px', padding: '0 2px' },
    deleted: { background: 'rgba(239,68,68,0.15)', color: '#f87171', borderRadius: '3px', padding: '0 2px', textDecoration: 'line-through' },
    paraphrased: { background: 'rgba(234,179,8,0.15)', color: '#fbbf24', borderRadius: '3px', padding: '0 2px' },
  }
  return <span style={styles[state]} title={title} aria-label={`${state}: ${text}`} className="text-sm">{text}{' '}</span>
}
...
export function DiffView() {
  ...
  const leftTokens = diff.filter(t => t.state === 'equal' || t.state === 'deleted' || t.state === 'paraphrased')
  const rightTokens = diff.filter(t => t.state === 'equal' || t.state === 'added' || t.state === 'paraphrased')
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="mb-2">
        <h1 className="text-2xl font-bold" style={{ color: 'var(--text-h)' }}>Model Output Diff View</h1>
        <p className="text-sm mt-1" style={{ color: 'var(--text)' }}>Token-level side-by-side comparison of two model versions.</p>
      </div>

      {/* Prompt */}
      <div className="rounded-2xl p-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
        <label htmlFor="diff-prompt" className="block text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--accent-2, #a78bfa)' }}>Prompt</label>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {[{ id: 'output-a', label: 'Model v1', value: outputA, set: (v: string) => { setOutputA(v); setDiff([]) } },
          { id: 'output-b', label: 'Model v2', value: outputB, set: (v: string) => { setOutputB(v); setDiff([]) } }
        ].map(({ id, label, value, set }) => (
          <div key={id} className="flex flex-col gap-2">
            <label htmlFor={id} className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--accent-2, #a78bfa)' }}>{label}</label>
            <textarea id={id} value={value} onChange={e => set(e.target.value)} placeholder={`Paste or simulate ${label} output…`} rows={4} style={inputStyle}
              onFocus={e => e.currentTarget.style.borderColor = 'var(--border-focus)'}
              onBlur={e => e.currentTarget.style.borderColor = 'var(--border)'}
            />
          </div>
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={() => { setPrompt(''); setOutputA(''); setOutputB(''); setDiff([]) }}
      ...
      {/* Summary */}
      {summary && (
        <div className="flex flex-wrap items-center gap-5 rounded-xl px-5 py-3" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }} aria-live="polite">
          {[
          ...
          {score !== null && (
            <span className="ml-auto text-sm font-semibold" style={{ color: 'var(--accent-2, #a78bfa)' }}>
              {score}% changed
            </span>
          )}
        </div>
      ...
      {diff.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
          {/* Legend */}
          <div className="flex gap-4 px-5 py-2.5 text-xs" style={{ background: 'var(--bg-card)', borderBottom: '1px solid var(--border)' }}>
            {[
              ...
              <span key={label} style={{ background: bg, color, borderRadius: '4px', padding: '1px 6px' }}>{label}</span>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 divide-x" style={{ divideColor: 'var(--border)' }}>
            {[
              ...
              <div key={side} className="p-5" style={{ background: 'var(--bg-card)' }}>
                <div className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--accent-2, #a78bfa)' }}>{label}</div>
                <div className="flex flex-wrap gap-y-1 leading-relaxed" role="region" aria-label={`${label} diff`}>
                  {tokens.map((token, i) => {
                    const text = side === 'left' ? (token.tokenA ?? '') : (token.tokenB ?? token.tokenA ?? '')
                    const title = token.state === 'paraphrased'
                      ? (side === 'left' ? `→ "${token.tokenB}" (sim: ${token.similarity?.toFixed(2)})` : `← "${token.tokenA}" (sim: ${token.similarity?.toFixed(2)})`)
                      : undefined
